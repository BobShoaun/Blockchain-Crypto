...
function calculateMempool(blockchain, headBlock, transactions) {
  ...
  return transactions.filter(
    (tx) =>
      !transactionSet.some((txSet) => txSet.hash === tx.hash) && !isCoinbase(tx)
  );
}
...
function calculateTransactionSet(blockchain, headBlock) {
  ...
  const prevTxSet = headBlock.previousHash
    ? calculateTransactionSet(
        blockchain,
        getPreviousBlock(blockchain, headBlock)
      )
    : [];
...
function createTransaction(params, inputs, outputs, message) {
  return {
    timestamp: Date.now(),
...
function getTxBlock(blockchain, headBlockHash, transaction) {
  ...
  for (let i = blockchain.length - 1; i >= 0; i--) {
    ...
    if (blockchain[i].transactions.some((tx) => tx.hash === transaction.hash))
      return blockchain[i];
  ...
  for (const transaction of transactions) {
    if (
      transaction.inputs.some((input) =>
        receivedTxs.some(
          (tx) =>
            input.txHash === tx.hash &&
            tx.outputs[input.outIndex].address === address
        )
      )
    )
